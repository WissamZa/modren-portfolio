...
const DEFAULT_COLOR = '#B19EEF';

// Create touch texture function (optimized)
const createTouchTexture = (): TouchTexture => {
  ...
  const drawPoint = (p: TrailPoint) => {
    ...
    ctx.shadowColor = `rgba(${r}, ${g}, ${b}, ${0.22 * intensity})`;
  ...
  const addTouch = (norm: { x: number; y: number }) => {
    if (last) {
      ...
      trail.push({ x: norm.x, y: norm.y, age: 0, force: 0, vx: 0, vy: 0 });
    }
...
// Optimized liquid effect creation
...
interface ThreeSetup {
  ...
  touch?: TouchTexture;
  liquidEffect?: Effect;
  trackedEffects: Effect[];
  trackedEffectPasses: EffectPass[];
}

// Type for pointer event mapping result
interface PointerMappingResult {
  fx: number;
  fy: number;
  w: number;
  h: number;
...
// Type for reinitialization config
...
const PixelBlast: React.FC<PixelBlastProps> = ({
  ...
  const mapToPixels = useCallback((e: PointerEvent, renderer: THREE.WebGLRenderer): PointerMappingResult => {
  ...
  const randomFloat = useCallback((): number => {
  ...
  useEffect(() => {
    ...
    if (mustReinit) {
      ...
      const trackedEffects: Effect[] = [];
      const trackedEffectPasses: EffectPass[] = [];
      ...
      if (liquid) {
        ...
        // Track the effect and pass
        trackedEffects.push(liquidEffect);
        trackedEffectPasses.push(effectPass);
      }
      
      // Setup noise effect if enabled
      if (noiseAmount > 0) {
        if (!composer) {
          composer = new EffectComposer(renderer);
        ...
        // Track the effect and pass
        trackedEffects.push(noiseEffect);
        trackedEffectPasses.push(noisePass);
      }
      ...
      const onPointerDown = (e: PointerEvent) => {
        ...
        const { fx, fy } = mapToPixels(e, threeRef.current.renderer);
      ...
      const onPointerMove = (e: PointerEvent) => {
        if (!touch || !threeRef.current) return;
        const { fx, fy, w, h } = mapToPixels(e, threeRef.current.renderer);
      ...
      const animate = () => {
        ...
        trackedEffects.forEach(eff => {
          if (eff.uniforms) {
            const u = eff.uniforms.get('uTime');
            if (u) u.value = currentTime;
          }
        });
      ...
      // Update liquid effect parameters
      if (t.liquidEffect) {
